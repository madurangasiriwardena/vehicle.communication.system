...
public class IssuingManager implements Serializable {
	...
	public Mpk getCertificate(String pseudo, BigInteger APrime, ECPoint h) {
		...
		BigInteger e;
		do {
			ePrime = new BigInteger(Constants.KePrime, Constants.certainty,
					new Random());
	...
	private BigInteger computeA(BigInteger aPrime, BigInteger e) {
		...
		// Euler function on n:
		BigInteger phiN = (this.getIsk().getp1().subtract(BigInteger.ONE))
				.multiply((this.getIsk().getp2().subtract(BigInteger.ONE)));
		// Compute 1/e
		BigInteger eInv = e.modInverse(phiN);
		BigInteger A = (aPrime.multiply(this.getIpk().geta0())).modPow(eInv,
				this.getIpk().getn());
		return A;
	}
