...
public class RevocationManager implements Serializable {
	...
	public BigInteger computeB(BigInteger ePrime) {
		...
		// Euler function on l:
		BigInteger phiL = (this.getRsk().getl1().subtract(BigInteger.ONE))
				.multiply((this.getRsk().getl2().subtract(BigInteger.ONE)));
		// Compute 1/e'
	...
	public void revokeUser(Mpk mpk) {
		...
		BigInteger bNew = this.getRpk().getb()
				.modPow(eInv, this.getRpk().getl());
	...
	public BigInteger getNewCertificat(Mpk mpk, RevocationCertificate certif) {
		...
		BigInteger alpha = certif.getMpk().getEPrime()
				.multiply(new BigInteger("3")).modInverse(phiL);
		BigInteger beta = mpk.getEPrime().multiply(new BigInteger("3"))
				.modInverse(phiL).multiply(Constants.TWO);
		// Update public key
		BigInteger B = (mpk.getB().modPow(alpha, rpk.getl()).multiply(certif
				.getRpk().getb().modPow(beta, rpk.getl()))).mod(rpk.getl());
		return B;
	}
